...
namespace klong {
    ...
    bool ResolveVisitor::resolveLocal(Variable* variable) {
        ...
        if (symbolInfoOptional.has_value()) {
            ...
            if (!_module->hasDependency(symbolInfo.owningModulepath)) {
                _session->reportError("Couldn't resolve variable '" + variable->name() + "'. But found a fitting definition in this other module " 
                    + symbolInfo.owningModulepath, variable->sourceRange());
                return false;
            }
        ...
        if (privateSymbols.size() == 1) {
            _session->reportError("Couldn't resolve variable '" + variable->name() + "'. But found a private definition in " + privateSymbols[0].declarationStmt->sourceRange().start.absolutepath(), variable->sourceRange());
            return false;
        }
        if (privateSymbols.size() > 1) {
            std::string paths = "";
            ...
            _session->reportError("Couldn't resolve variable '" + variable->name() + "'. But found multiple private definitions in:\n" + paths, variable->sourceRange());
            return false;
        }

        _session->reportError("Couldn't resolve variable '" + variable->name() + "'.", variable->sourceRange());
        return false;
    }
    ...
    void ResolveVisitor::declare(Stmt* declarationStmt, std::string name, DeclarationType declarationType, bool isPublic) {
        ...
        if (!_isInsideFunction) {
            if (isPublic) {
                if (!_session->declareSymbol(name, symbolInfo)) {
                    _session->reportError("Symbol with name '" + name + "' already declared in global scope", declarationStmt->sourceRange());
                }
            } else {
        ...
        if (scope.find(name) != scope.end()) {
            _session->reportError("Symbol with name '" + name + "' already declared in this scope", declarationStmt->sourceRange());
        }
    ...
    void ResolveVisitor::visitModule(Module* module) {
        for (auto& stmt : module->statements()) {
            if (stmt->kind() != StatementKind::FUNCTION 
                ...
                _session->reportError("Illegal top level statement.", stmt->sourceRange());
            }
        }
    ...
    void ResolveVisitor::visitReturnStmt(Return* stmt) {
        if (!_isInsideFunction) {
            _session->reportError("Cannot return from top-level code.", stmt->sourceRange());
        }
        resolve(stmt->value());
    }
    ...
    void ResolveVisitor::visitAssignExpr(Assign* expr) {
        ...
        if (expr->target()->kind() == ExprKind::VARIABLE) {
            ...
            if (isResolved && varDeclRes->kind() == StatementKind::VAR_DECL) {
                ...
                if (varDecl->isConst()) {
                    _session->reportError("Cannot reassign 'const'.", expr->sourceRange());
                }
            }
        } else {
            resolve(expr->target());
    ...
    void ResolveVisitor::visitVariableExpr(Variable* expr) {
        ...
        if (scope.find(expr->name()) != scope.end()) {
            ...
            if (!symbol.initialized) {
                _session->reportError("Cannot read local variable in its own initializer.", expr->sourceRange());
            }
        }
        resolveLocal(expr);
    }
