...
#include "common/compilation_incident.h"
...
namespace klong {
    std::multimap<char, Lexer::LexerCaseCallable> Lexer::cases = {
		...
		{'e', std::bind(&Lexer::enumKeyword, std::placeholders::_1, std::placeholders::_2)},
        ...
        {'d', std::bind(&Lexer::deferKeyword, std::placeholders::_1, std::placeholders::_2)},
    ...
    Token Lexer::next() {
        ...
        if (!hasFoundMatchingCase) {
            ...
            throw CompilationIncident(CompilationIncidentType::ERROR, "Illegal char sequence encountered.", token.sourceRange);
        }
	...
	bool Lexer::enumKeyword(Token& token) {
		return matchesKeyword(token, "enum", TokenType::ENUM);
	}
    ...
    bool Lexer::deferKeyword(Token& token) {
        return matchesKeyword(token, "defer", TokenType::DEFER);
    }
